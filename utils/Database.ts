...
const db: SQLiteDB = openDatabaseSync('app.db');

export async function migrateDbIfNeeded(db: SQLiteDB) {
...
export const addChat = async (db: SQLiteDB, title: string) => {
  return await db.runAsync('INSERT INTO chats (title) VALUES (?)', title);
};

export const getChats = async (db: SQLiteDB) => {
  return await db.getAllAsync('SELECT * FROM chats');
};

export const getMessages = async (db: SQLiteDB, chatId: number): Promise<Message[]> => {
...
export const deleteChat = async (db: SQLiteDB, chatId: number) => {
  return await db.runAsync('DELETE FROM chats WHERE id = ?', chatId);
};

export const renameChat = async (db: SQLiteDB, chatId: number, title: string) => {
